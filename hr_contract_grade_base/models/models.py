...
class HRGradeConfiguration(models.Model):
    ...
    min_amount = fields.Integer("Min.")
    max_amount = fields.Integer("Max.")
    inc_amount = fields.Integer("Increament")
    job_ids  = fields.Many2many('hr.job', 'grade_rel', 'grade_id', 'job_id', string='Job Positions')
    ...
    def _check_job_positions(self):
        if self.grade_id and self.grade_id.job_ids:
            if self.job_id not in self.grade_id.job_ids:
                raise UserError(_("Employee Job Title is not Allowed To Take This Grade"))
